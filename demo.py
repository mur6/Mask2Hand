...
def demo(model, device="cpu"):
    rh_model = mano.load(model_path="./models/MANO_RIGHT.pkl", is_rhand=True, num_pca_comps=45, flat_hand_mean=True)
    ...
    with torch.no_grad():
        ...
        camera_Ks = np.array(
            [[focal_lens[0, 0].cpu(), 0.0, 112.0], [0.0, focal_lens[0, 1].cpu(), 112.0], [0.0, 0.0, 1.0]]
        )
        ...
        save_a_image_with_mesh_joints(
            rgb_input[0],
            mask_bgr,
            (silhouettes[0] == 0).cpu().numpy().astype("uint8") * 255,
            contour,
            camera_Ks,
            verts[0].cpu().numpy(),
            faces[0].cpu().numpy(),
...
def test():
    ...
    # Create model
    model = HandSilhouetteNet3(mano_model_path="./models/MANO_RIGHT.pkl", num_pca_comps=45, device=device)

    # Demo
    checkpoint_path = "checkpoint/model_best_4.pth"
    checkpoint = torch.load(checkpoint_path, map_location="cpu")
    model.load_state_dict(checkpoint["model_state_dict"])
    check_epoch = checkpoint["epoch"]
    print("=" * 10, f"Model: {checkpoint_path} (Epoch: {check_epoch})", "=" * 10)
